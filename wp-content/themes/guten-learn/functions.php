<?php
/**
 * Guten Learn functions and definitions
 *
 * Guten Learn only works in WordPress 4.7 or later.
 *
...
define( 'GUTEN_LEARN_PREFIX', 'gutenlearn' );
define( 'GUTEN_LEARN_VERSION', '1.0.3' );

require get_parent_theme_file_path( '/classes/class-helper.php' );

final class Guten_Learn_Theme extends Guten_Learn_Helper{
	/**
	 * constructor
	 *
	 * @since 1.0.0
	...
	*/
	public static function customize_register( $wp_customize ){
		...
		$wp_customize->get_control( 'header_textcolor' )->label = esc_html__( 'Site Identity Color', 'guten-learn' );
		...
		$wp_customize->get_section( 'header_image' )->title = esc_html__( 'Inner Banner Options', 'guten-learn' );
	...
	*/
	public static function meta_fields(){
		...
		$label = esc_html__( 'Guten Learn Settings', 'guten-learn' );
		foreach ( $post_types as $type ) {

			$post = new Guten_Learn_Meta_Fields( $type );
			$options = array(
				'sidebar-position' => array(
					...
					'default' => 'customizer',
					'choices' => array(
						'customizer' 	=> esc_html__( 'From customizer', 'guten-learn' ),
						'left-sidebar' 	=> esc_html__( 'Left', 'guten-learn' ),
						'right-sidebar' => esc_html__( 'Right', 'guten-learn' ),
						'no-sidebar' 	=> esc_html__( 'Hide', 'guten-learn' ),
					),
				),
			);
			...
			if( 'page' == $type ){
				$page_options = array(
					'disable-inner-banner' => array(
						'type'  => 'checkbox',
						'label' => esc_html__( 'Disable Banner', 'guten-learn' ),
					),
					'disable-footer-widget' => array(
						'type'  => 'checkbox',
						'label' => esc_html__( 'Disable Footer Widget', 'guten-learn' ),
					),
				);
	...
	*/
	public static function the_inner_banner_content( ){
		...
		if( is_home() || is_front_page() ){
			if( !guten_learn_get( 'show-slider' ) ){
				return;
			}

			get_template_part( 'templates/content/content', 'banner-slider' );
		}else{
	...
	*/
	public static function excerpt( $e ){
		...
		$more = sprintf( '<a href="%1$s">%2$s<i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>',
	        ...
	        esc_html__( 'Read More', 'guten-learn' )
	    );
	...
	*/
	public static function get_excerpt_length() {
		return guten_learn_get( 'excerpt_length' );
	}
	...
	public static function get_blog_title(){
		...
		$ib_blog_title = guten_learn_get( 'ib-blog-title' );
	...
	public static function show_breadcrumb(){
		...
		return guten_learn_get( 'show-breadcrumb' );
	}
	...
	public static function show_preloader(){
		return guten_learn_get( 'pre-loader' );
	}
	...
	public static function show_post_author(){
		return guten_learn_get( 'post-author' );
	}
	...
	public static function show_post_date(){
		return guten_learn_get( 'post-date' );
	}
	...
	public static function show_post_category(){
		return guten_learn_get( 'post-category' );
	}
	...
	public static function supports(){
		 ...
		 */
		load_theme_textdomain( 'guten-learn', self::get_theme_path( 'languages' ) );
	}
	...
	public static function sidebars(){
		...
		register_sidebar( array(
	        'name' 			=> esc_html__( 'Sidebar', 'guten-learn' ),
	        'id' 			=> self::fn_prefix( 'sidebar' ),
	        'description' 	=> esc_html__( 'Widgets in this area will be shown on side of the page. If leaved empty some default widget added by default.', 'guten-learn' ),
	        'before_widget' => '<section id="%1$s" class="widget %2$s">',
	        'after_widget'  => '</section>',
	        'before_title'  => '<h2 class="widget-title">',
	        'after_title'   => '</h2>',
		));
		$description = esc_html__( 'Widgets in this area will be displayed in the {position} column in the footer. If leaved empty some default widget added by default', 'guten-learn' );
		for( $i = 1; $i <= guten_learn_get( 'layout-footer' ); $i++ ){
			switch ($i){
				case '1':
					$position = esc_html__( 'first', 'guten-learn' );
				break;
				case '2':
					$position = esc_html__( 'second', 'guten-learn' );
				break;
				case '3':
					$position = esc_html__( 'third', 'guten-learn' );
				break;
				case '4':
					$position = esc_html__( 'fourth', 'guten-learn' );
				break;
				default:
					$position = esc_html__( 'first', 'guten-learn' );
			...
			register_sidebar( array(
		        ...
		        'name' 			=> sprintf( esc_html__( 'Footer Widget Area %d ', 'guten-learn' ), $i ),
	...
	public static function nav_menu(){
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Primary', 'guten-learn' ),
				'social-menu-footer' => esc_html__( 'Footer social menu', 'guten-learn' )
			)
		);
	}
	...
	public static function get_sidebar_class( $classes ){
		...
		if( $page_template || is_search() || self::is_woo_single_page() || self::is_woo_product_category() ){
			...
			$customizer_position = guten_learn_get( 'sidebar-position' );
	...
	public static function inner_banner_class( $classes ){
		$classes[] = guten_learn_get( 'ib-text-align' );
		if( has_header_image() ){
			$classes[] = guten_learn_get( 'ib-image-attachment' );
		}
	...
	public static function get_body_classes ( $classes ){
		# Container class
		if( 'box' == guten_learn_get( 'container-width' ) ){
			$classes[] = self::with_prefix( 'container-box' );
		}

		$post_per_row = guten_learn_get( 'post-per-row' );
		if( '1' == $post_per_row ){
			$classes[] = self::with_prefix( 'post-per-page-one' );
		}

		#add class for header layout
		$header_layout = guten_learn_get( 'layout-header' );
		
		if( '1' == $header_layout ){
			$classes[] = self::with_prefix( 'header-logo-left' );
		}elseif( '2' == $header_layout ){
			$classes[] = self::with_prefix( 'header-logo-center' );
		}elseif( '3' == $header_layout){
			$classes[] = self::with_prefix( 'header-logo-right' );
		}
	...
	public static function register_required_plugins(){
		$plugins = array(
			array(
				'name'     => esc_html__( 'Rise Blocks - A Complete Gutenberg Page builder', 'guten-learn' ),
				'slug'     => 'rise-blocks',
				'required' => false,
			),
		);
	...
	*/
	public static function footer_social_menu(){
		if( !guten_learn_get( 'footer-social-menu' ) ){
			return;
		}
	...
	*/
	 ...
	 * @package Guten Learn WordPress Theme
	 */
	public static function get_posts_by_type( $type, $cat_id, $post_per_page = 3 ){
		$post_pp = apply_filters( Guten_Learn_Helper::fn_prefix( 'slider_post_per_page' ), $post_per_page );
		$posts = array();
		if( 'post' == $type ){
	...
	*/
	public static function the_default_image( $post_id = false ){
		...
		if( $post_id && has_post_thumbnail( $post_id ) ){
			...
			echo 'background-color:'. guten_learn_get( 'primary-color' );
		}
	}
	...
	*/
	public static function the_default_search(){ ?>
        <section class="widget widget_search">
			<h2 class="widget-title"><?php esc_html_e( 'Search', 'guten-learn' ); ?></h2>	        	
           <?php get_search_form(); ?>
        </section>
	<?php }
	...
	*/
	public static function the_default_recent_post(){ ?>
        <section class="widget widget_recent_entries">
			<h2 class="widget-title"><?php esc_html_e( 'Recent Posts', 'guten-learn' ); ?></h2>
			<?php $pst = Guten_Learn_Theme::get_posts_by_type( 'post', 0, 5 ); 
			if( !empty( $pst ) ):?>
				<ul>
	...
	*/
	public static function the_default_archive(){ ?>
		<section class="widget">
		    <h3 class="widget-title"><?php esc_html_e( 'Archives', 'guten-learn' ); ?></h3>
		    <ul>
